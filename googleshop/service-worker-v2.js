...
async function handleGetCapturedDownloads(message) {
  ...
  let downloads = capturedDownloads.get(message.tabId) || [];
  
  // Also check storage
  try {
    const result = await chrome.storage.local.get(['capturedDownloads']);
    if (result.capturedDownloads && result.capturedDownloads[message.tabId]) {
      downloads = result.capturedDownloads[message.tabId];
    }
  } catch (e) {
    console.error('[Service Worker] Error reading captured downloads from storage:', e);
  }
...
chrome.downloads.onCreated.addListener((downloadItem) => {
  ...
  console.log('[Service Worker] Download captured:', downloadItem.filename);
  console.log('[Service Worker] Download URL:', downloadItem.url);
  console.log('[Service Worker] Final URL:', downloadItem.finalUrl);
  
  // Extract best filename from URL if downloadItem.filename is missing
  ...
  if (!filename || filename === '') {
    ...
    try {
      ...
      // Look for last segment that looks like a file (has extension)
      ...
      // If still no filename, use last segment or fallback
      if (!filename) {
        filename = segments[segments.length - 1] || 'installer.exe';
      }
    } catch (e) {
      console.error('[Service Worker] Error extracting filename from URL:', e);
      filename = 'installer.exe';
    }
  }
  
  const download = { url: downloadItem.url, filename: filename };
  ...
  // Save to storage so popup can retrieve it
  chrome.storage.local.get(['capturedDownloads'], (result) => {
    const stored = result.capturedDownloads || {};
    stored[captureTabId] = capturedDownloads.get(captureTabId);
    chrome.storage.local.set({ capturedDownloads: stored });
  });
  
  // Try to send to popup (may fail if popup is closed - that's OK)
  chrome.runtime.sendMessage({
    type: 'DOWNLOAD_CAPTURED',
    tabId: captureTabId,
    download: download
  }).catch(() => {
    console.log('[Service Worker] Popup not open, download saved to storage');
  });
  
  // Auto-stop capture after first download
  captureTabId = null;
});
